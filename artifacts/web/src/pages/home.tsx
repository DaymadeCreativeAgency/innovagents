  ...
  return (
    <Layout>
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#0f0e0b]">
        {/* Subtle ambient blobs — hero only */}
        ...
        {/* Bottom fade */}
        <div className="absolute bottom-0 left-0 right-0 h-24 bg-gradient-to-t from-[#0f0e0b] to-transparent pointer-events-none" />
      </section>

      {/* ── STATS ── */}
      <section className="border-y border-white/[0.06] bg-[#151311] py-12">
      ...
      {/* ── PROBLEM STATEMENT ── */}
      <section className="py-32 bg-[#0f0e0b]">
        <div className="max-w-6xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 24 }}
          ...
          >
            <h2 className="text-4xl md:text-6xl text-white leading-[1.05]">
            ...
            <p className="text-base text-white/30 max-w-3xl mx-auto leading-relaxed">
      ...
      {/* ── APPS SHOWCASE ── */}
      <section id="apps" className="py-32 bg-[#151311]">
        <div className="max-w-6xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {APPS.map((app, i) => (
              <motion.div
                key={i}
                variants={fadeUp}
                className={`group relative bg-white/[0.035] border rounded-3xl p-6 transition-all duration-300 ${
                  app.available
              ...
              >
                <div className="flex items-start justify-between mb-5">
                  ...
                  {app.available ? (
                    <span className="text-[10px] font-semibold bg-primary/[0.15] text-primary border border-primary/[0.22] px-3 py-1 rounded-full">
                      Available
                    </span>
                  ) : (
                    <span className="text-[10px] font-semibold bg-white/[0.06] text-white/30 border border-white/[0.08] px-3 py-1 rounded-full">
                      Coming Soon
                    </span>
                  )}
                </div>
                <h3 className="text-white font-display font-medium text-xl mb-2">{app.name}</h3>
                <p className="text-white/38 text-sm leading-relaxed mb-5">{app.desc}</p>
                {app.available && app.href && (
                  <div className="flex gap-2.5">
                    <Button
                      asChild
                      size="sm"
                      className="flex-1 text-xs h-8 bg-primary/[0.15] hover:bg-primary/25 text-primary border border-primary/[0.22] rounded-full transition-colors"
                    >
                    ...
                    <Button
                      asChild
                      size="sm"
                      className="flex-1 text-xs h-8 bg-transparent text-white/50 hover:text-white border border-white/[0.09] hover:bg-white/[0.06] rounded-full transition-colors"
                    >
                      <Link href={app.href}>
            ...
            {WHY_US.map((f, i) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-white/38 leading-relaxed text-sm">{f.desc}</p>
                </div>
              </motion.div>
            ))}
          </motion.div>
      ...
      {/* ── NEWSLETTER ── */}
      <section className="py-32 bg-[#151311]">
        <div className="max-w-6xl mx-auto px-6">
          <motion.div
            initial={{ opacity: 0, y: 24 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="bg-white/[0.04] border border-white/[0.09] rounded-3xl p-10 md:p-14">
              ...
              {newsletterSuccess ? (
                <motion.div
                  ...
                  className="bg-primary/[0.12] border border-primary/[0.25] text-primary p-4 rounded-2xl flex items-center justify-center gap-3"
                >
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col sm:flex-row gap-2.5">
                        ...
                        <FormItem className="flex-1">
                          <FormControl>
                            <Input
                              ...
                              {...field}
                              className="h-11 bg-white/[0.06] border-white/[0.11] text-white placeholder:text-white/25 focus:border-primary/40 transition-colors rounded-full px-5"
                            />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                    ...
                    <Button
                      type="submit"
                      className="h-11 px-7 bg-primary hover:bg-primary/90 text-white font-semibold rounded-full transition-colors shadow-sm shrink-0"
                    >
